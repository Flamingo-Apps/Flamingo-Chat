...
- **Logging**: structured JSON logs from every service (not plain text), shipped via Promtail into Loki, viewable in the same Grafana instance as metrics and traces. This is optional for the very first phase - `kubectl logs` is enough while there's one replica per service - but worth wiring up before there's more than a couple of pods, since that's when `kubectl logs` per-pod stops being usable.
  - **Foundation already in place** (built during Identity Service, ahead of this phase): a shared gRPC unary interceptor (`pkg/grpclog`) logs one structured line per request (method, duration, status code) via `log/slog`, currently to stdout. This is deliberately the same choke point tracing/metrics will extend - a trace span starts/ends around the same `handler(ctx, req)` call the interceptor already wraps, and a Prometheus duration histogram observes the same `duration` this interceptor already computes. When this phase is built, it's grpclog gaining OTel span creation and a metrics recorder, not a new mechanism being introduced.
